...
Engine::Engine() : dvc(&instance, &surface, &device) {};
...
void Engine::run() {
	...
	dvc.pickPhysicalDevice();
    dvc.createLogicalDevice(validationLayers, graphicsQueue, presentQueue);
    Swapchain swp(&surface, &device, &dvc.physicalDevice, glfw_s.window);
    swp.createSwapChain();
