...
public:
    ...
    bool checkValidationLayerSupport() {
        ...
        for (const char* layerName : base->validationLayers) {
...
public:
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
        ...
        for (const auto& queueFamily : queueFamilies) {
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(device, i, base->surface, &presentSupport);
    ...
    SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, base->surface, &details.capabilities);

        uint32_t formatCount;
        vkGetPhysicalDeviceSurfaceFormatsKHR(device, base->surface, &formatCount, nullptr);

        if (formatCount != 0) {
            details.formats.resize(formatCount);
            vkGetPhysicalDeviceSurfaceFormatsKHR(device, base->surface, &formatCount, details.formats.data());
        }

        uint32_t presentModeCount;
        vkGetPhysicalDeviceSurfacePresentModesKHR(device, base->surface, &presentModeCount, nullptr);

        if (presentModeCount != 0) {
            details.presentModes.resize(presentModeCount);
            vkGetPhysicalDeviceSurfacePresentModesKHR(device, base->surface, &presentModeCount, details.presentModes.data());
        }
...
public:
    ...
    void createSwapChain() {
        ...
        QueueFamilyIndices indices = dvc->findQueueFamilies(dvc->physicalDevice);
        ...
        if (vkCreateSwapchainKHR(dvc->device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
            throw std::runtime_error("failed to create swap chain!");
        }

        vkGetSwapchainImagesKHR(dvc->device, swapChain, &imageCount, nullptr);
        swapChainImages.resize(imageCount);
        vkGetSwapchainImagesKHR(dvc->device, swapChain, &imageCount, swapChainImages.data());
    ...
    VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
        ...
        if (vkCreateImageView(dvc->device, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
    ...
    void createRenderPass() {
        ...
        depthAttachment.format = dvc->findDepthFormat();
        depthAttachment.samples = dvc->msaaSamples;
    ...
    void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
        ...
        if (vkCreateImage(dvc->device, &imageInfo, nullptr, &image) != VK_SUCCESS) {
            throw std::runtime_error("failed to create image!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(dvc->device, image, &memRequirements);

        VkMemoryAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocInfo.allocationSize = memRequirements.size;
        allocInfo.memoryTypeIndex = dvc->findMemoryType(memRequirements.memoryTypeBits, properties);

        if (vkAllocateMemory(dvc->device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate image memory!");
        }

        vkBindImageMemory(dvc->device, image, imageMemory, 0);
    }
...
public:
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) {
        ...
        if (vkCreateShaderModule(dvc->device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
    ...
    void createGraphicsPipeline() {
        ...
        fragShaderStageInfo.pName = "main";// "main";
        ...
        multisampling.rasterizationSamples = dvc->msaaSamples;
        ...
        if (vkCreatePipelineLayout(dvc->device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        ...
        pipelineInfo.renderPass = img->renderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

        if (vkCreateGraphicsPipelines(dvc->device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < img->swapChainImageViews.size(); i++) {
            std::array<VkImageView, 3> attachments = {
                img->colorImageView,
                img->depthImageView,
                img->swapChainImageViews[i]
            };

            VkFramebufferCreateInfo framebufferInfo{};
            framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
            framebufferInfo.renderPass = img->renderPass;
            framebufferInfo.attachmentCount = static_cast<uint32_t>(attachments.size());
            framebufferInfo.pAttachments = attachments.data();
            framebufferInfo.width = img->swapChainExtent.width;
            framebufferInfo.height = img->swapChainExtent.height;
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
        ...
        if (vkCreateBuffer(dvc->device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
            throw std::runtime_error("failed to create buffer!");
        }

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(dvc->device, buffer, &memRequirements);
        ...
        if (vkAllocateMemory(dvc->device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate buffer memory!");
        }

        vkBindBufferMemory(dvc->device, buffer, bufferMemory, 0);
    }
    ...
    VkCommandBuffer beginSingleTimeCommands() {
        ...
        vkAllocateCommandBuffers(dvc->device, &allocInfo, &commandBuffer);
    ...
    void endSingleTimeCommands(VkCommandBuffer commandBuffer) {
        ...
        vkQueueSubmit(dvc->graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
        vkQueueWaitIdle(dvc->graphicsQueue);

        vkFreeCommandBuffers(dvc->device, commandPool, 1, &commandBuffer);
    }
    ...
    void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
        ...
        vkGetPhysicalDeviceFormatProperties(dvc->physicalDevice, imageFormat, &formatProperties);
    ...
    void createTextureImage() {
        ...
        void* data;
        vkMapMemory(dvc->device, stagingBufferMemory, 0, imageSize, 0, &data);
        memcpy(data, pixels, static_cast<size_t>(imageSize));
        vkUnmapMemory(dvc->device, stagingBufferMemory);

        stbi_image_free(pixels);

        img->createImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, img->textureImage, textureImageMemory);

        transitionImageLayout(img->textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
        copyBufferToImage(stagingBuffer, img->textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
    ...
    void createVertexBuffer() {
        ...
        void* data;
        vkMapMemory(dvc->device, stagingBufferMemory, 0, bufferSize, 0, &data);
    ...
    void createDescriptorSets() {
        ...
        if (vkAllocateDescriptorSets(dvc->device, &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(dvc->device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
                vkCreateSemaphore(dvc->device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS ||
                vkCreateFence(dvc->device, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        renderPassInfo.renderPass = img->renderPass;
        renderPassInfo.framebuffer = swapChainFramebuffers[imageIndex];
        renderPassInfo.renderArea.offset = { 0, 0 };
        renderPassInfo.renderArea.extent = img->swapChainExtent;
        ...
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        viewport.width = (float)img->swapChainExtent.width;
        viewport.height = (float)img->swapChainExtent.height;
        ...
        VkRect2D scissor{};
        scissor.offset = { 0, 0 };
        scissor.extent = img->swapChainExtent;
    ...
    void drawFrame() {
        ...
        if (vkQueueSubmit(dvc->graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
        ...
        VkSwapchainKHR swapChains[] = { img->swapChain };
